...
from nlp_processing_beto import correct_transcription_with_beto  # Importar la corrección con BETO
...
def transcribe_segment(audio_path, start_time, end_time):
    ...
    # Usar directamente la transcripción de Whisper y aplicar la corrección con BETO
    transcription_text = result['text']
    corrected_text = correct_transcription_with_beto(transcription_text)
...
for audio_file in os.listdir(PROCESSED_AUDIO_DIR):
    if audio_file.endswith('.wav'):
        ...
        # Eliminar el archivo de audio procesado
        os.remove(audio_path)  # Eliminar el archivo de audio original una vez que se ha procesado
